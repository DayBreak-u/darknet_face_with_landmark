...
YOLOFACE::YOLOFACE()
{
    ...
    yolofacenet.load_param("../model/mbv2_yolov3_face.param");
    yolofacenet.load_model("../model/mbv2_yolov3_face.bin");
                ...
                if (confidence >= confidence_threshold)
                    {
                        ...
                        float lx1 = (j + ptr_lx1[0]) / w;
                        float ly1 = (i + ptr_ly1[0]) / h;
                        float lx2 = (j + ptr_lx2[0]) / w;
                        float ly2 = (i + ptr_ly2[0]) / h;
                        float lx3 = (j + ptr_lx3[0]) / w;
                        float ly3 = (i + ptr_ly3[0]) / h;
                        float lx4 = (j + ptr_lx4[0]) / w;
                        float ly4 = (i + ptr_ly4[0]) / h;
                        float lx5 = (j + ptr_lx5[0]) / w;
                        float ly5 = (i + ptr_ly5[0]) / h;
...
//    float w_scale = im_bgr.cols * 1.0 / im.cols;
    ...
    ex.extract("633", pred3); // 48
    ex.extract("692", pred2); // 32
    ex.extract("751", pred1); // 64
